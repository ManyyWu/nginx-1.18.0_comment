...
/*
nginx这里采用了64个slot时间,也就是每次更新时间的时候都是更新下一个slot,
如果读操作同时进行,读到的还是之前的slot,并没有被改变,当然这里只能是尽量减少了时间混乱的几率,因为slot的个数不是无限的,slot是循环的,
写操作总有几率会写到读操作的slot上. 不过nginx现在实际上并没有采用多线程的方式,而且在信号处理中只是更新cached_err_log_time,
...
┃                                ┃点0分0秒到某一时间的秒数,tp          ┃下面会说明ngx_tm_t类型                         ┃
┃(time_t t, ngx_tm_t *tp)        ┃                                  ┃                                             ┃
┃                                ┃是ngx_tm_t类型的时间,实际上         ┃                                              ┃
┃                                ┃就是标准的tm类型时间                ┃                                              ┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╋━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╋━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃                                ┃                                  ┃    返回一1表示失败,否则会返回：①如             ┃
...
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┻━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┻━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
                     ...
                     +-----------------+
                     | ngx_cached_time | <-- writer
                     +-----------------+
                           /     \
                          /       \
                         /  --->   \  (ngx_cached_time 指针移动方向)
                        /           \
+---------+---------+---------+---------+---------+--------------------------+
| slot[0] | slot[1] | slot[2] | slot[3] |   ...   | slot[NGX_TIME_SLOTS - 1] |
+---------+---------+---------+---------+---------+--------------------------+
                       ^
                       |
                     reader
...
为避免分裂读,即某worker进程读时间缓存过程中接受中断请求,期间时间缓存被其他worker更新,导致前后读取时间不一致;
nginx引入时间缓存数组(共64个成员),每次都更新数组中的下一个元素;
更新时间通过ngx_time_update()实现,ngx_time_update()调用最频繁的是在worker进程处理事件时
...
ngx_time_update(void)
{
    ...
    /*
    可以看到ngx_memory_barrier()之后是四条赋值语句,如果没有 ngx_memory_barrier(),编译器可能会将
        ngx_cached_time = tp;
    ...
    分别和之前的
        ...
        p1 = &cached_err_log_time[slot][0];
        p2 = &cached_http_log_time[slot][0];
    合并优化掉,这样的后果是ngx_cached_time,ngx_cached_http_time,ngx_cached_err_log_time,ngx_cached_http_log_time这四个时间缓存的
    不一致性时长增大了,因为在最后一个ngx_sprintf执行完后这四个时间缓存才一致,在这之前如果有其他地方正在读时间缓存就可能导致读到的时间
    不正确或者不一致,而采用ngx_memory_barrier() 后,时间缓存更新到一致的状态只需要几个时钟周期,因为只有四条赋值指令,显然在这么短的时
    间内发生读时间缓存的概率会小的多了. 从这里可以看出Igor考虑是非常细致的.
    */
    /* 禁止编译器对后面的语句优化,如果没有这个限制,编译器可能将前后两部分代码合并,可能导致这6个时间更新出现间隔,期间若被读取会出现时间不一致的情况 */
    ngx_memory_barrier();
