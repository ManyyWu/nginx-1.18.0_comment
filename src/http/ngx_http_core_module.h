...
typedef struct {
    ...
    socklen_t                  socklen; //一般在设备listen中设置了bind setfib, backlog, rcvbuf, sndbuf, accept_filter, deferred, ipv6only, or so_keepalive这些参数后会把bind和set一起置1
    ngx_str_t                  addr_text;
...
*/
    unsigned                   ssl:1;
    unsigned                   http2:1;  //listen是否启用http2配置,例如listen 443 ssl http
#if (NGX_HAVE_INET6)
    unsigned                   ipv6only:1;
#endif
...
typedef ngx_int_t (*ngx_http_output_body_filter_pt)
    (ngx_http_request_t *r, ngx_chain_t *chain); //见ngx_http_top_body_filter
