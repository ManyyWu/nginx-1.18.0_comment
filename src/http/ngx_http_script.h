...
typedef struct { //创建空间赋值见ngx_http_rewrite
    ...
    unsigned                    test:1; //我是要看看是否正则匹配成功,你待会匹配的时候记得放个变量到堆栈里.  if{}配置的时候需要
    unsigned                    negative_test:1;
    unsigned                    uri:1; //是否是URI匹配.   rewrite配置时会置1表示需要进行uri匹配
    unsigned                    args:1;

    /* add the r->args to the new arguments */
    unsigned                    add_args:1; //是否自动追加参数到rewrite后面. 如果目标结果串后面用问好结尾,则nginx不会拷贝参数到后面的

    //rewrite aaa bbb op;中的bbb为"http://" "https://" "$scheme"或者op为redirect permanent则redirect置1,见ngx_http_rewrite
    unsigned                    redirect:1; //nginx判断,如果是用http://等开头的rewrite,就代表是垮域重定向. 会做302处理.
...
typedef struct { //ngx_http_rewrite中分配空间,赋值
    ...
    /* add the r->args to the new arguments */
    unsigned                    add_args:1; //和ngx_http_script_regex_code_t->add_args相等,见ngx_http_rewrite

    unsigned                    redirect:1; //和ngx_http_script_regex_code_t->redirect相等,见ngx_http_rewrite
