    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (kcf->original_init_upstream(cf, us) != NGX_OK) { //默认ngx_http_upstream_init_round_robi
        return NGX_ERROR;
    }
    ...
    ngx_uint_t state)
{
    ...
    if (!u->keepalive) { //说明本次和后端的连接使用的是缓存cache(keepalive配置)connection的TCP连接,也就是使用的是之前已经和后端建立好的TCP连接ngx_connection_t
        goto invalid;
    }
