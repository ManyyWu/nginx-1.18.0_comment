...
//所以对conf赋值就是对ngx_cycle_s中的conf_ctx赋值
    ...
    */ 
    for (m = 0; cf->cycle->modules[m]; m++) { //注意这里为所有的NGX_HTTP_MODULE开辟了main_conf srv_conf loc_conf空间,也就是在http{}的时候为所有main srv loc开辟了空间
        if (cf->cycle->modules[m]->type != NGX_HTTP_MODULE) { //http{}相关配置结构创建首先需要执行ngx_http_core_module,而后才能执行对应的http子模块
            continue;
        }

        module = cf->cycle->modules[m]->ctx;
        mi = cf->cycle->modules[m]->ctx_index; //mi实际上是依次递增的,见签名的ctx_index赋值处
    ...
    for (m = 0; cf->cycle->modules[m]; m++) { //按照模块类型进行合并  http{} server{} location{}都属于同一个ngx_http_core_module模块,他们的init_main_conf都是一样的
        if (cf->cycle->modules[m]->type != NGX_HTTP_MODULE) {
            continue;
        }
